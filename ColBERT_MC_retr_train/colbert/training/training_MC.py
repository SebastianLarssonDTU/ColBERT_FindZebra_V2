...
def train(args):
    ...
    triples_w_options = ["correct_answer.tsv",
                         "option1.tsv",
                         "option2.tsv",
                         "option3.tsv",
                         "option4.tsv"]
    ...
    for i in range(args.num_options):
        triples_path = args.triples + triples_w_options[i]
        if args.lazy:
            reader = LazyBatcher(args, (0 if args.rank == -1 else args.rank), args.nranks)
        else:
            reader = EagerBatcher(args, (0 if args.rank == -1 else args.rank), args.nranks, triples_path)
...
#Added by me | Un-tested | Freeze the layers when fine tuning the model | https://huggingface.co/transformers/training.html
    ...
    labels_opts = torch.zeros(args.num_options, dtype=torch.long, device=DEVICE)
    ...
    for batch_idx, _ in zip(range(start_batch_idx, args.maxsteps), readers[0]):
        loss_opts = []
        for reader in readers:
            _, BatchSteps = zip(range(start_batch_idx, args.maxsteps), reader)[batch_idx]
        ...
        loss_opts = torch.Tensor(loss_opts).permute(1,0).cuda()
        loss = criterion(loss_opts, labels_opts)
